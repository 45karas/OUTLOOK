...
# Render Deployment for This Fork

This repository includes a `render.yaml` blueprint for deploying GraphSpy as a Render web service.

1. Push this repository to GitHub.
2. In Render, create a new Blueprint and select this repository.
3. Render will run `pip install .` and start the app with `graphspy -i 0.0.0.0 -p $PORT`.
4. Open the Render service URL after the deploy finishes.

## Company Microsoft Login

This fork can send users through Microsoft's real OAuth sign-in and open the Outlook Graph panel automatically.

Set these Render environment variables:

- `MS_TENANT_ID` - your tenant ID, or `common` for multi-tenant testing
...
- `FLASK_SECRET_KEY` - generated by Render or any long random value
